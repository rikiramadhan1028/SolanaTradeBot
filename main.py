...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        welcome_text, reply_markup=get_start_menu_keyboard(user_id)
    )
...
async def handle_wallet_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard_buttons.append(
        [
            InlineKeyboardButton(f"Create Solana Wallet", callback_data=f"create_wallet:solana"),
    ...
    await query.edit_message_text(
        "Wallet Options:", reply_markup=InlineKeyboardMarkup(keyboard_buttons)
    )
    ...
    if command == "import":
        if len(args) == 0:
            await update.message.reply_text(
                "❌ Invalid Format Use: `import [private_key]`", parse_mode="Markdown"
            )
            return
        try:
            ...
            try:
                pubkey = wallet_manager.get_solana_pubkey_from_private_key_json(
                    cleaned_key
                )
            except Exception as e:
            ...
            await update.message.reply_text(
                "❌ An unexpected error occurred during import. Please check your private key format."
            )
        return
    ...
    if command == "sendtoken":
        try:
            ...
            if not wallet or not wallet["private_key"]:
                await update.message.reply_text("❌ No Solana wallet found.")
                return
            tx = solana_client.send_spl_token(
                wallet["private_key"], token_addr, to_addr, amount
            )
            if tx and not tx.lower().startswith("error"):
                solscan_link = f"https://solscan.io/tx/{tx}"
                await update.message.reply_text(
...
async def back_to_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_text = await get_dynamic_start_message_text(user_id, user_mention)
    await query.edit_message_text(
        welcome_text, reply_markup=get_start_menu_keyboard(user_id), parse_mode="HTML"
    )
...
async def handle_delete_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"🗑️ Your Solana wallet has been deleted.",
        reply_markup=InlineKeyboardMarkup(
...
async def buy_sell(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.answer()
    keyboard = [
        [InlineKeyboardButton("Buy", callback_data="dummy_buy"),
         InlineKeyboardButton("Sell", callback_data="dummy_sell")],
        ...
        [InlineKeyboardButton("📉 Limit Orders", callback_data="dummy_limit_orders"),
         InlineKeyboardButton("Auto Sell", callback_data="dummy_auto_sell")],
        [InlineKeyboardButton("📈 Positions", callback_data="dummy_positions"),
         InlineKeyboardButton("👛 Wallet", callback_data="dummy_wallet"),
         InlineKeyboardButton("❓ Help", callback_data="dummy_help")],
        [InlineKeyboardButton("💵 Smart Wallet", callback_data="dummy_smart_wallet"),
         InlineKeyboardButton("🖥️ Extension", callback_data="dummy_extension")],
        [InlineKeyboardButton("⚙️ Settings", callback_data="dummy_settings"),
         InlineKeyboardButton("💰 Referrals", callback_data="dummy_referrals")],
        [InlineKeyboardButton("⬅️ Back to Menu", callback_data="back_to_main_menu")],
    ]
...
async def handle_buy_sell_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action.startswith("buy_fixed_"):
        ...
        keyboard = [[InlineKeyboardButton("⬅️ Cancel", callback_data="back_to_main_menu")]]
        await query.edit_message_text(
            "Please enter the amount of SOL you want to buy with:", reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
async def perform_trade(update: Update, context: ContextTypes.DEFAULT_TYPE, amount):
    ...
    await message.reply_text(
        f"⏳ Performing {trade_type} of token `{token_address}` on {dex.capitalize()}..."
    )
    ...
    if tx_sig.startswith("Error"):
        ...
        await message.reply_text(
            f"✅ Swap successful! View transaction: https://solscan.io/tx/{tx_sig}",
            parse_mode="Markdown",
        )
    clear_user_context(context)
    await message.reply_text(
...
# =========================
# Pumpfun Auto Trade (Local/Bundle)
# =========================
...
async def handle_pumpfun_token_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        ...
        [InlineKeyboardButton("🟢 Buy (Jito Bundle)", callback_data="pf_buy_jito"),
         InlineKeyboardButton("🔴 Sell (Jito Bundle)", callback_data="pf_sell_jito")],
        [InlineKeyboardButton("❌ Batal", callback_data="back_to_main_menu")],
    ]
    await update.message.reply_text(
