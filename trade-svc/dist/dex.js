...
function keyBytes(input) {
    const s = (input || '').trim();
    if (!s)
        throw new Error('empty privateKey');
    try {
        if (s.startsWith('['))
...
function mapAxiosErr(e) {
    const ae = e;
    if (ae?.response)
        return `HTTP ${ae.response.status} ${JSON.stringify(ae.response.data).slice(0, 300)}`;
    if (ae?.request)
...
async function ensureSufficientBalances(opts) {
    ...
    if (balLamports < required) {
        ...
        throw new Error(`balance_low: need ~${(required / 1e9).toFixed(6)} SOL, have ${(balLamports / 1e9).toFixed(6)} SOL. ` +
    ...
    if (inputMint !== NATIVE_SOL) {
        try {
            ...
            if (totalRaw < BigInt(amountRaw)) {
                throw new Error(`token_balance_low: have ${totalRaw.toString()} raw, need ${amountRaw} raw for input mint ${inputMint}`);
            }
        }
        catch (e) {
...
export async function dexSwap(p) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            const vtx = signFromBase64Unsigned(txB64, user);
