...
import { getSolBalance, getTokenBalances, getSpecificTokenBalance, getMintDecimals, } from './wallet.js';
...
function chooseCuPrice(tier) {
    if (!tier)
        return DEX_CU_PRICE_MICRO_DEFAULT || undefined;
    const t = String(tier).toLowerCase();
    if (t === 'fast')
        return DEX_CU_PRICE_MICRO_FAST;
    if (t === 'turbo')
        return DEX_CU_PRICE_MICRO_TURBO;
    if (t === 'ultra')
        return DEX_CU_PRICE_MICRO_ULTRA;
    return DEX_CU_PRICE_MICRO_DEFAULT || undefined;
}
const app = express();
...
app.get('/diag/ping', async (_req, res) => {
    try {
...
app.get('/diag/jup', async (_req, res) => {
    try {
        const lite = await dns.lookup('lite-api.jup.ag', { all: true, verbatim: true });
        let pro = null;
        try {
            pro = await dns.lookup('api.jup.ag', { all: true, verbatim: true });
        }
        catch { }
...
app.get('/diag/quote', async (req, res) => {
    try {
        ...
        let lastStatus = 500;
        let lastBody = { error: 'quote_failed' };
...
app.post('/settings/priority', (req, res) => {
    ...
    if (typeof value === 'number' && value >= 0) {
        if (tier === 'fast') {
            DEX_CU_PRICE_MICRO_FAST = value;
            return res.json({ fast: value });
        }
        if (tier === 'turbo') {
            DEX_CU_PRICE_MICRO_TURBO = value;
            return res.json({ turbo: value });
        }
        if (tier === 'ultra') {
            DEX_CU_PRICE_MICRO_ULTRA = value;
            return res.json({ ultra: value });
        }
...
app.post('/pumpfun/swap', async (req, res) => {
    const { privateKey, action, mint, amount, useJito = false, bundleCount = 1, slippage = 10, priorityFee = 0.0001, // Default fallback - will be overridden by client tier system
    pool, // Tambahkan pool
     } = req.body || {};
...
// ---------- Wallet (web3.js) ----------
app.get('/wallet/:addr/balance', async (req, res) => {
    try {
...
app.get('/wallet/:addr/tokens', async (req, res) => {
    try {
...
app.get('/wallet/:addr/token/:mint/balance', async (req, res) => {
    try {
...
app.get('/wallet/mint/:mint/decimals', async (req, res) => {
    try {
