...
export async function getSolBalance(rpcUrl, owner) {
    const c = conn(rpcUrl);
    const pk = new PublicKey(owner);
    const lamports = await c.getBalance(pk, { commitment: 'confirmed' });
    return lamports / LAMPORTS_PER_SOL;
}
export async function getTokenBalances(rpcUrl, owner) {
    const c = conn(rpcUrl);
    const pk = new PublicKey(owner);
    async function fetchByProgram(programId, label) {
    ...
    const [spl, t22] = await Promise.all([
        ...
        fetchByProgram(TOKEN_2022_PROGRAM_ID, 'token2022').catch(() => []),
    ]);
...
export async function getSpecificTokenBalance(rpcUrl, owner, mint) {
    const c = conn(rpcUrl);
    ...
    for (const { account } of accs.value) {
        ...
        if (!ta)
            continue;
        if (ta.uiAmount != null)
            total += Number(ta.uiAmount);
        else if (ta.uiAmountString != null)
            total += Number(ta.uiAmountString);
        else {
...
export async function getMintDecimals(rpcUrl, mint) {
    const c = conn(rpcUrl);
    const mintPk = new PublicKey(mint);
    try {
